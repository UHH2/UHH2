...
██   ██ ███████ ██   ██ ██████  ██      ██ ███████     ███████    ██    ██   ██ ██   ██    ██
*/
// UHH2 note: This is a hacked version of hist/hist/src/TFormula.cxx of ROOT v6.18.04 - Do not use this for anything but UHH2BTagCalibReader unless you really know what you are doing!
...
static const TString gNamePrefix = "UHH2TFormula__";
...
// static void R__v5UHH2TFormulaUpdater(Int_t nobjects, TObject **from, TObject **to)
// {
//    auto **fromv5 = (ROOT::v5::TFormula **)from;
//    auto **target = (UHH2TFormula **)to;
//
//    for (int i = 0; i < nobjects; ++i) {
//       if (fromv5[i] && target[i]) {
//          UHH2TFormula fnew(fromv5[i]->GetName(), fromv5[i]->GetExpFormula());
//          *(target[i]) = fnew;
...
// }
//
// using UHH2TFormulaUpdater_t = void (*)(Int_t nobjects, TObject **from, TObject **to);
// bool R__SetClonesArrayUHH2TFormulaUpdater(UHH2TFormulaUpdater_t func);
//
// int R__RegisterUHH2TFormulaUpdaterTrigger = R__SetClonesArrayUHH2TFormulaUpdater(R__v5UHH2TFormulaUpdater);

////////////////////////////////////////////////////////////////////////////////
Bool_t UHH2TFormula::IsOperator(const char c)
{
...
////////////////////////////////////////////////////////////////////////////////
Bool_t UHH2TFormula::IsBracket(const char c)
{
...
////////////////////////////////////////////////////////////////////////////////
Bool_t UHH2TFormula::IsFunctionNameChar(const char c)
{
   return !IsBracket(c) && !IsOperator(c) && c != ',' && c != ' ';
}

////////////////////////////////////////////////////////////////////////////////
Bool_t UHH2TFormula::IsDefaultVariableName(const TString &name)
{
   return name == "x" || name == "z" || name == "y" || name == "t";
}

////////////////////////////////////////////////////////////////////////////////
Bool_t UHH2TFormula::IsScientificNotation(const TString & formula, int i)
{
...
////////////////////////////////////////////////////////////////////////////////
Bool_t UHH2TFormula::IsHexadecimal(const TString & formula, int i)
{
   // check if the character at position i  is part of a scientific notation
...
////
Bool_t UHH2TFormula::IsAParameterName(const TString & formula, int pos) {
...
////////////////////////////////////////////////////////////////////////////////
bool UHH2TFormulaParamOrder::operator() (const TString& a, const TString& b) const {
...
////////////////////////////////////////////////////////////////////////////////
void UHH2TFormula::ReplaceAllNames(TString &formula, map<TString, TString> &substitutions)
{
...
UHH2TFormula::~UHH2TFormula()
{
   ...
   if (!TestBit(UHH2TFormula::kNotGlobal) && gROOT ) {
...
////////////////////////////////////////////////////////////////////////////////
UHH2TFormula::UHH2TFormula(const char *name, const char *formula, bool addToGlobList, bool vectorize)   :
   TNamed(name,formula),
   fClingInput(formula),fFormula(formula)
{
   ...
   if (addToGlobList && gROOT) {
      UHH2TFormula *old = 0;
      R__LOCKGUARD(gROOTMutex);
      old = dynamic_cast<UHH2TFormula*> ( gROOT->GetListOfFunctions()->FindObject(name) );
      if (old)
         gROOT->GetListOfFunctions()->Remove(old);
      if (IsReservedName(name))
         Error("UHH2TFormula","The name %s is reserved as a UHH2TFormula variable name.\n",name);
      else
...
UHH2TFormula::UHH2TFormula(const char *name, const char *formula, int ndim, int npar, bool addToGlobList)   :
   TNamed(name,formula),
   fClingInput(formula),fFormula(formula)
{
   fReadyToExecute = false;
   ...
   if (ret)  {
      ...
      Error("UHH2TFormula","Syntax error in building the lambda expression %s", formula );
}

////////////////////////////////////////////////////////////////////////////////
UHH2TFormula::UHH2TFormula(const UHH2TFormula &formula) :
   TNamed(formula.GetName(),formula.GetTitle()), fMethod(nullptr)
{
   ...
   if (!TestBit(UHH2TFormula::kNotGlobal) && gROOT ) {
      R__LOCKGUARD(gROOTMutex);
      UHH2TFormula *old = (UHH2TFormula*)gROOT->GetListOfFunctions()->FindObject(formula.GetName());
      if (old)
         gROOT->GetListOfFunctions()->Remove(old);
...
UHH2TFormula& UHH2TFormula::operator=(const UHH2TFormula &rhs)
{
...
////////////////////////////////////////////////////////////////////////////////
Bool_t UHH2TFormula::InitLambdaExpression(const char * formula) {
...
Int_t UHH2TFormula::Compile(const char *expression)
{
   ...
   if (TestBit(UHH2TFormula::kLambda) ) {
...
////////////////////////////////////////////////////////////////////////////////
void UHH2TFormula::Copy(TObject &obj) const
{
   TNamed::Copy(obj);
   // need to copy also cling parameters
   UHH2TFormula & fnew = dynamic_cast<UHH2TFormula&>(obj);
   ...
   // copy Linear parts (it is a vector of UHH2TFormula pointers) needs to be copied one by one
   ...
   if (nLinParts > 0) {
      ...
      for (int i = 0; i < nLinParts; ++i) {
         UHH2TFormula * linearNew = new UHH2TFormula();
         UHH2TFormula * linearOld = (UHH2TFormula*) fLinearParts[i];
         if (linearOld) {
   ...
   if (fLambdaPtr && TestBit(UHH2TFormula::kLambda)) {

      bool ret = fnew.InitLambdaExpression(fnew.fFormula);
      if (ret)  {
         fnew.SetBit(UHH2TFormula::kLambda);
         fnew.fReadyToExecute = true;
      }
      else {
         Error("UHH2TFormula","Syntax error in building the lambda expression %s", fFormula.Data() );
...
void UHH2TFormula::Clear(Option_t * )
{
   fNdim = 0;
   fNpar = 0;
   fNumber = 0;
...
/// UHH2TFormula uses it to execute function.
/// After call, UHH2TFormula should be ready to evaluate formula.
/// Returns false on failure.

bool UHH2TFormula::PrepareEvalMethod()
{
   if (!fMethod) {
...
void UHH2TFormula::InputFormulaIntoCling()
{
   ...
   if (!fClingInitialized && fReadyToExecute && fClingInput.Length() > 0) {
      ...
      TString triggerAutoparsing = "namespace ROOT_UHH2TFormula_triggerAutoParse {\n"; triggerAutoparsing += fClingInput + "\n}";
...
void UHH2TFormula::FillDefaults()
{
   ...
   for (const auto &var : defvars) {
      int pos = fVars.size();
      fVars[var] = UHH2TFormulaVariable(var, 0, pos);
   ...
   //    fVars[xvar] =  UHH2TFormulaVariable(xvar,0,i);
   //    fClingVariables.push_back(0);
   // }
...
void UHH2TFormula::FillVecFunctionsShurtCuts() {
...
void UHH2TFormula::HandlePolN(TString &formula)
{
...
///    `UHH2TFormula::FillParametrizedFunctions` defined further below:
///
...
void UHH2TFormula::HandleParametrizedFunctions(TString &formula)
{
...
void UHH2TFormula::HandleParamRanges(TString &formula)
{
...
void UHH2TFormula::HandleFunctionArguments(TString &formula)
{
   ...
   for (Int_t i = 0; i < formula.Length(); ++i) {
      // List of things to ignore (copied from `UHH2TFormula::ExtractFunctors`)
      ...
      if (isalpha(formula[i]) && !IsOperator(formula[i])) {
         ...
         UHH2TFormula *f = dynamic_cast<UHH2TFormula *>(obj);
         if (!f) {
            // maybe object is a TF1
            // Error("HandleFunctionArguments", "UHH2 hack: the following three lines of code had to be commented out because the TF1 class is not known in this scope!");
...
void UHH2TFormula::HandleExponentiation(TString &formula)
{
...
void UHH2TFormula::HandleLinear(TString &formula)
{
   ...
   while (linPos != kNPOS && !IsAParameterName(formula, linPos)) {
      ...
      formula.ReplaceAll(pattern, replacement);
      if (first) {
         UHH2TFormula *lin1 = new UHH2TFormula("__linear1", left, false);
         fLinearParts.push_back(lin1);
      }
      UHH2TFormula *lin2 = new UHH2TFormula("__linear2", right, false);
...
void UHH2TFormula::PreProcessFormula(TString &formula)
{
   formula.ReplaceAll("**","^");
...
Bool_t UHH2TFormula::PrepareFormula(TString &formula)
{
   fFuncs.clear();
   fReadyToExecute = false;
...
void UHH2TFormula::ExtractFunctors(TString &formula)
{
   ...
   for (Int_t i = 0; i < formula.Length(); ++i) {
      ...
      if (formula[i] == '[') {
         ...
         fFuncs.push_back(UHH2TFormulaFunction(param));
          ...
          !IsOperator(formula[i])) // not really needed to check if operator (if isalpha is not an operator)
      {
         ...
         if (formula[i] == '(') {
            i++;
            if (formula[i] == ')') {
               fFuncs.push_back(UHH2TFormulaFunction(name, body, 0));
               name = body = "";
               continue;
            }
            Int_t depth = 1;
            ...
            fFuncs.push_back(UHH2TFormulaFunction(name, body, args));
         } else {
            ...
            if (!f) {
               // maybe object is a TF1
               // Error("ExtractFunctors", "UHH2 hack: the following three lines of code had to be commented out because the TF1 class is not known in this scope!");
               // TF1 *f1 = dynamic_cast<TF1 *>(obj);
               // if (f1)
               //    f = f1->GetFormula();
            }
            ...
            i += 2;
            fFuncs.push_back(UHH2TFormulaFunction(name));
         }
      }
      name = body = "";
   }
...
void UHH2TFormula::ProcessFormula(TString &formula)
{
   // std::cout << "Begin: formula is " << formula << " list of functors " << fFuncs.size() << std::endl;

   for (list<UHH2TFormulaFunction>::iterator funcsIt = fFuncs.begin(); funcsIt != fFuncs.end(); ++funcsIt) {
      UHH2TFormulaFunction &fun = *funcsIt;
      ...
      if (fun.IsFuncCall()) {
         ...
         if (!fun.fFound) {
            ...
            if (gDebug)
               Info("UHH2TFormula", "Could not find %s function with %d argument(s)", fun.GetName(), fun.GetNargs());
            fun.fFound = false;
         }
#endif
      } else {
         ...
         {
            R__LOCKGUARD(gROOTMutex);
            old = (UHH2TFormula *)gROOT->GetListOfFunctions()->FindObject(gNamePrefix + fun.fName);
         }
         if (old) {
         ...
         map<TString, UHH2TFormulaVariable>::iterator varsIt = fVars.find(fun.GetName());
         if (varsIt != fVars.end()) {
            ...
            if (digit >= fNdim) {
               ...
               for (int j = 0; j < fNdim; ++j) {
                  ...
                  if (fVars.find(vname) == fVars.end()) {
                     fVars[vname] = UHH2TFormulaVariable(vname, 0, j);
   ...
   if (!fReadyToExecute) {
      ...
      if (inputIntoCling) {
         ...
         if (funcit != gClingFunctions.end()) {
            fFuncPtr = (UHH2TFormula::CallFuncSignature)funcit->second;
            fClingInitialized = true;
   ...
   if (!fClingInitialized && !fLazyInitialization) {
      ...
      for (list<UHH2TFormulaFunction>::iterator it = fFuncs.begin(); it != fFuncs.end(); ++it) {
...
void UHH2TFormula::FillParametrizedFunctions(map<pair<TString, Int_t>, pair<TString, TString>> &functions)
{
   // map< pair<TString,Int_t> ,pair<TString,TString> > functions;
   functions.insert(
...
void UHH2TFormula::SetPredefinedParamNames() {
...
const TObject* UHH2TFormula::GetLinearPart(Int_t i) const
{
...
void UHH2TFormula::AddVariable(const TString &name, double value)
{
   if (fVars.find(name) != fVars.end()) {
      UHH2TFormulaVariable &var = fVars[name];
      var.fValue = value;
      ...
      UHH2TFormulaVariable var(name, value, fVars.size());
      fVars[name] = var;
...
void UHH2TFormula::AddVariables(const TString *vars, const Int_t size)
{
   ...
   for (Int_t i = 0; i < size; ++i) {
      ...
      UHH2TFormulaVariable &var = fVars[vname];
      if (var.fArrayPos < 0) {
...
void UHH2TFormula::SetName(const char* name)
{
   if (IsReservedName(name)) {
      Error("SetName", "The name \'%s\' is reserved as a UHH2TFormula variable name.\n"
...
void UHH2TFormula::SetVariables(const pair<TString,Double_t> *vars, const Int_t size)
{
   for(Int_t i = 0; i < size; ++i)
      {
      auto &v = vars[i];
...
Double_t UHH2TFormula::GetVariable(const char *name) const
{
...
Int_t UHH2TFormula::GetVarNumber(const char *name) const
{
   const auto nameIt = fVars.find(name);
   if (fVars.end() == nameIt) {
...
TString UHH2TFormula::GetVarName(Int_t ivar) const
{
...
void UHH2TFormula::SetVariable(const TString &name, Double_t value)
{
   if (fVars.find(name) == fVars.end()) {
...
void UHH2TFormula::DoAddParameter(const TString &name, Double_t value, Bool_t processFormula)
{
   ...
   if(fParams.find(name) != fParams.end() )
      ...
      // UHH2TFormulaVariable & par = fParams[name];
      // par.fValue = value;
      if (ipos < 0) {
         ipos = fParams.size();
         fParams[name] = ipos;
      ...
      fNpar++;
      // UHH2TFormulaVariable(name,value,fParams.size());
      int pos = fParams.size();
      // fParams.insert(std::make_pair<TString,UHH2TFormulaVariable>(name,UHH2TFormulaVariable(name,value,pos)));
...
Int_t UHH2TFormula::GetParNumber(const char * name) const {
...
Double_t UHH2TFormula::GetParameter(const char * name) const
{
...
Double_t UHH2TFormula::GetParameter(Int_t param) const
{
...
const char * UHH2TFormula::GetParName(Int_t ipar) const
{
...
////////////////////////////////////////////////////////////////////////////////
Double_t* UHH2TFormula::GetParameters() const
{
...
void UHH2TFormula::GetParameters(Double_t *params) const
{
...
void UHH2TFormula::SetParameter(const char *name, Double_t value)
{
   ...
   fAllParametersSetted = true;
   for (map<TString, UHH2TFormulaVariable>::iterator it = fParams.begin(); it != fParams.end(); ++it) {
      if (!it->second.fFound) {
         fAllParametersSetted = false;
         break;
      }
...
void UHH2TFormula::SetParameters(const pair<TString,Double_t> *params,const Int_t size)
{
   for(Int_t i = 0 ; i < size ; ++i)
      {
...
////////////////////////////////////////////////////////////////////////////////
void UHH2TFormula::DoSetParameters(const Double_t *params, Int_t size)
{
...
void UHH2TFormula::SetParameters(const Double_t *params)
{
...
void UHH2TFormula::SetParameters(Double_t p0, Double_t p1, Double_t p2, Double_t p3, Double_t p4, Double_t p5, Double_t p6,
...
void UHH2TFormula::SetParameter(Int_t param, Double_t value)
{
...
////////////////////////////////////////////////////////////////////////////////
void UHH2TFormula::SetParNames(const char *name0, const char *name1, const char *name2, const char *name3,
...
////////////////////////////////////////////////////////////////////////////////
void UHH2TFormula::SetParName(Int_t ipar, const char * name)
{
   ...
   if (! TestBit(UHH2TFormula::kLambda))  ReplaceParamName(fFormula, oldName, name);
...
void UHH2TFormula::ReplaceParamName(TString & formula, const TString & oldName, const TString & name){
   if (!formula.IsNull() ) {
      bool found = false;
      for(list<UHH2TFormulaFunction>::iterator it = fFuncs.begin(); it != fFuncs.end(); ++it)
         {
...
////////////////////////////////////////////////////////////////////////////////
void UHH2TFormula::SetVectorized(Bool_t vectorized)
{
#ifdef R__HAS_VECCORE
   if (fNdim == 0) {
...
////////////////////////////////////////////////////////////////////////////////
Double_t UHH2TFormula::EvalPar(const Double_t *x,const Double_t *params) const
{
   if (!fVectorized)
...
bool UHH2TFormula::fIsCladRuntimeIncluded = false;
...
bool UHH2TFormula::GenerateGradientPar()
{
   ...
   if (!HasGradientGenerationFailed()) {
      ...
      if (!UHH2TFormula::fIsCladRuntimeIncluded) {
         UHH2TFormula::fIsCladRuntimeIncluded = true;
         gInterpreter->Declare("#include <Math/CladDerivator.h>\n#pragma clad OFF");
      }

      // Check if the gradient request was made as part of another UHH2TFormula.
      // This can happen when we create multiple UHH2TFormula objects with the same
      ...
      if (!functionExists(GetGradientFuncName())) {
         ...
         // We want to call clad::differentiate(UHH2TFormula_id);
...
void UHH2TFormula::GradientPar(const Double_t *x, UHH2TFormula::GradientStorage& result)
{
...
void UHH2TFormula::GradientPar(const Double_t *x, Double_t *result)
{
   void* args[3];
   ...
   if (fNpar <= 0) {
      ...
      // {
      //    if (ret) {
      //       new (ret) (double) (((double (&)(double*))UHH2TFormula____id)(*(double**)args[0]));
      //       return;
      //    } else {
      //       ((double (&)(double*))UHH2TFormula____id)(*(double**)args[0]);
      //       return;
      //    }
      // }
      args[1] = &result;
      ...
      // {
      //    ((void (&)(double*, double*,
      //               double*))UHH2TFormula____id_grad)(*(double**)args[0], *(double**)args[1],
      //                                                                 *(double**)args[2]);
      //    return;
      // }
...
////////////////////////////////////////////////////////////////////////////////
#ifdef R__HAS_VECCORE
// ROOT::Double_v UHH2TFormula::Eval(ROOT::Double_v x, ROOT::Double_v y, ROOT::Double_v z, ROOT::Double_v t) const
// {
//    ROOT::Double_v xxx[] = {x, y, z, t};
//    return EvalPar(xxx, nullptr);
// }

ROOT::Double_v UHH2TFormula::EvalParVec(const ROOT::Double_v *x, const Double_t *params) const
{
   if (fVectorized)
...
Double_t UHH2TFormula::Eval(Double_t x, Double_t y, Double_t z, Double_t t) const
{
...
Double_t UHH2TFormula::Eval(Double_t x, Double_t y , Double_t z) const
{
   double xxx[3] = {x,y,z};
   return EvalPar(xxx, nullptr);
}
...
Double_t UHH2TFormula::Eval(Double_t x, Double_t y) const
{
   double xxx[2] = {x,y};
   return EvalPar(xxx, nullptr);
}
...
Double_t UHH2TFormula::Eval(Double_t x) const
{
   double * xxx = &x;
   return EvalPar(xxx, nullptr);
}
...
Double_t UHH2TFormula::DoEval(const double * x, const double * params) const
{
   if(!fReadyToExecute)
      {
      Error("Eval", "Formula is invalid and not ready to execute ");
      for (auto it = fFuncs.begin(); it != fFuncs.end(); ++it) {
         UHH2TFormulaFunction fun = *it;
         if (!fun.fFound) {
            printf("%s is unknown.\n", fun.GetName());
         }
      }
   ...
   if (!fClingInitialized && fLazyInitialization) {
      ...
      R__LOCKGUARD(gROOTMutex);
      auto thisFormula = const_cast<UHH2TFormula*>(this);
   ...
   if (fLambdaPtr && TestBit(UHH2TFormula::kLambda)) {// case of lambda functions
...
#ifdef R__HAS_VECCORE
ROOT::Double_v UHH2TFormula::DoEvalVec(const ROOT::Double_v *x, const double *params) const
{
   if (!fReadyToExecute) {
      Error("Eval", "Formula is invalid and not ready to execute ");
      for (auto it = fFuncs.begin(); it != fFuncs.end(); ++it) {
...
////////////////////////////////////////////////////////////////////////////////
void UHH2TFormula::ReInitializeEvalMethod() {
   ...
   if (!fLazyInitialization)   Warning("ReInitializeEvalMethod", "Formula is NOT properly initialized - try calling again UHH2TFormula::PrepareEvalMethod");
...
TString UHH2TFormula::GetExpFormula(Option_t *option) const
{
   TString opt(option);
   if (opt.IsNull() || TestBit(UHH2TFormula::kLambda) ) return fFormula;
   opt.ToUpper();
...
TString UHH2TFormula::GetGradientFormula() const {
   Error("GetGradientFormula", "UHH2 hack: TInterpreter::MakeInterpreterValue() not known in this ROOT version. Following three lines of code replace by dummy return value");
...
void UHH2TFormula::Print(Option_t *option) const
{
   ...
   if (opt.Contains("V") ) {
      if (fNdim > 0 && !TestBit(UHH2TFormula::kLambda)) {
   ...
   if(!fReadyToExecute)
      ...
      for (list<UHH2TFormulaFunction>::const_iterator it = fFuncs.begin(); it != fFuncs.end(); ++it) {
         UHH2TFormulaFunction fun = *it;
         if (!fun.fFound) {
            printf("%s is unknown.\n", fun.GetName());
         }
   ...
   if (!fAllParametersSetted) {
      ...
      // for(map<TString,UHH2TFormulaVariable>::const_iterator it = fParams.begin(); it != fParams.end(); ++it)
      // {
      //    pair<TString,UHH2TFormulaVariable> param = *it;
...
/*
void UHH2TFormula::Streamer(TBuffer &b)
{
   if (b.IsReading() ) {
      UInt_t R__s, R__c;
      ...
      if (v <= 8 && v > 3 && v != 6) {
         // old UHH2TFormula class
         ROOT::v5::TFormula * fold = new ROOT::v5::TFormula();
         // read old TFormula class
         fold->Streamer(b, v,  R__s, R__c, UHH2TFormula::Class());
         //std::cout << "read old tformula class " << std::endl;
         UHH2TFormula fnew(fold->GetName(), fold->GetExpFormula() );

         *this = fnew;

         //          printf("copying content in a new UHH2TFormula \n");
         ...
         // new UHH2TFormula class
         b.ReadClassBuffer(UHH2TFormula::Class(), this, v, R__s, R__c);
      ...
      // case of writing
      b.WriteClassBuffer(UHH2TFormula::Class(), this);
